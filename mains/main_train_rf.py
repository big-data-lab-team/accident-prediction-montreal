...
i = 1
while isdir(workdir + f'data/rf_{i}'):
    i += 1
mkdir(workdir + f'data/rf_{i}')
...
write_params(model, workdir + f'data/rf_{i}/params')
...
# Write results
with open(workdir + f'data/rf_{i}/results', 'w') as file:
...
feature_importances.to_csv(workdir + f'data/rf_{i}/feature_importances.csv')

# Write precision recall curve
precision_recall = compute_precision_recall_graph(predictions, 20)
precision_recall.to_csv(workdir + f'data/rf_{i}/precision_recall.csv')
